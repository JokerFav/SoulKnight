...
void Object::render_behind(Window &main_window, float y)
{
	SDL_Rect p_in, p_out;
...
void Object::render_front(Window &main_window, float y)
{
	SDL_Rect p_in, p_out;
	for(int i = 0; i < (int)pos.size(); ++i)
		if(pos[i].y > y)
...
Decorations::Decorations(Window &main_window):
